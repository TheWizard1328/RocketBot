...
namespace RocketBot2.Forms
{
    internal partial class SettingsForm : System.Windows.Forms.Form
    {
        ...
        private void SettingsForm_Load(object sender, EventArgs e)
        {
            ...
            gMapCtrl.Position = new PointLatLng(_session.Settings.AccountLatitude, _session.Settings.AccountLongitude);
...
#pragma warning restore IDE0018 // Inline variable declaration - Build.Bat Error Happens if We Do
        ...
        { 
            ...
            DeviceIdTb.Text = device.DeviceInfo.DeviceId;
            AndroidBoardNameTb.Text = device.DeviceInfo.AndroidBoardName;
            AndroidBootloaderTb.Text = device.DeviceInfo.AndroidBootloader;
            DeviceBrandTb.Text = device.DeviceInfo.DeviceBrand;
            DeviceModelTb.Text = device.DeviceInfo.DeviceModel;
            DeviceModelIdentifierTb.Text = device.DeviceInfo.DeviceModelIdentifier;
            DeviceModelBootTb.Text = device.DeviceInfo.DeviceModelBoot;
            HardwareManufacturerTb.Text = device.DeviceInfo.HardwareManufacturer;
            HardwareModelTb.Text = device.DeviceInfo.HardwareModel;
            FirmwareBrandTb.Text = device.DeviceInfo.FirmwareBrand;
            FirmwareTagsTb.Text = device.DeviceInfo.FirmwareTags;
            FirmwareTypeTb.Text = device.DeviceInfo.FirmwareType;
            FirmwareFingerprintTb.Text = device.DeviceInfo.FirmwareFingerprint;
        }
        ...
        public void ReInitializeSession(ISession session, Account requestedAccount = null)
        {
            if (session.LogicSettings.MultipleBotConfig.StartFromDefaultLocation)
            {
                var CurrentAltitude = new GeoCoordinate(session.Settings.AccountLatitude, session.Settings.AccountLongitude).Altitude;
                session.ReInitSessionWithNextBot(requestedAccount, session.Settings.AccountLatitude, session.Settings.AccountLongitude, CurrentAltitude);
            }
            else
            {
        ...
        private void SaveBtn_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                _session.Settings.AccountLatitude = Convert.ToDouble(tbLatitude.Text);
                _session.Settings.AccountLongitude = Convert.ToDouble(tbLongitude.Text);
        ...
        private void ResetLocationBtn_Click(object sender, EventArgs e)
        {
            ...
            UpdateMapLocation(_session.Settings.AccountLatitude, _session.Settings.AccountLongitude);
        }
