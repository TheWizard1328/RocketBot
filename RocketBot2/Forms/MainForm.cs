...
namespace RocketBot2.Forms
{
    public partial class MainForm : System.Windows.Forms.Form
    {
        ...
        public static void ColoredConsoleWrite(Color color, string text)
        {
            ...
            if (text.Contains($"Hash API server (https://pokehash.buddyauth.com/{Configuration.HashEndpoint}) might down!"))
        ...
        {
            var lat = _session.Settings.AccountLatitude;
            var lng = _session.Settings.AccountLongitude;
        ...
        private async Task InitializePokestopsAndRoute()
        {
            ...
            try
            {
                var mapObjects = _session.Client.ClientSession.Map.Cells;
                List<FortData> forts = new List<FortData>(mapObjects.SelectMany(p => p.Forts).ToList());
                ...
                //get optimized route
                pokeStops = new List<FortData>(RouteOptimizeUtil.Optimize(sessionForts.ToArray(), _session.Client.ClientSession.Player.Latitude, _session.Client.ClientSession.Player.Longitude));
            }
            catch
            {
                return;
        ...
        private void Navigation_UpdatePositionEvent()
        {
            ...
            SynchronizationContext.Post(o =>
            {
                ...
                _session.Settings.AccountLatitude = _currentLatLng.Lat;
                _session.Settings.AccountLongitude = _currentLatLng.Lng;
        ...
        private async void GMapControl1_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            if (!_botStarted)
            {
                ...
                _session.Settings.AccountLatitude = pos.Lat;
                _session.Settings.AccountLongitude = pos.Lng;
        ...
        private async void SettingsStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            var newLocation = new PointLatLng(_session.Settings.AccountLatitude, _session.Settings.AccountLongitude);
            double Dist = LocationUtils.CalculateDistanceInMeters(_session.Settings.AccountLatitude, _session.Settings.AccountLongitude, newLocation.Lat, newLocation.Lng);
            ...
            if (!_botStarted && Dist > 0)
            {
                ...
                _session.Settings.AccountLatitude = newLocation.Lat;
                _session.Settings.AccountLongitude = newLocation.Lng;
                ...
                _session.Client.ClientSession.Player.SetCoordinates(newLocation.Lat, newLocation.Lng, Alt);
        ...
        private async Task ReloadPokemonList()
        {
            ...
            try
            {
                var templates = _session.Client.ClientSession.Templates.ItemTemplates.Where(x => x.PokemonSettings != null)
                ...
                Instance.lblPokemonList.Text = _session.Translation.GetTranslation(TranslationString.AmountPkmSeenCaught, _totalData, 252, _totalCaptures, 252) +
                    $" | Storage: {_session.Client.ClientSession.Player.Data.MaxPokemonStorage} (Pokémons: {pokemons.Count()}, Eggs: {_session.Inventory.GetEggs().Result.Count()}) [Depolyments: {NewCount}]";
                ...
                if (itemscount != OldItemsCount)
                {
                    ...
                    Instance.lblInventory.Text =
                            $"Types: {items.Count()} | Total: {_session.Inventory.GetTotalItemCount().Result} | Storage: {_session.Client.ClientSession.Player.Data.MaxItemStorage}";
                }
        ...
        private void InitializeBot(Action<ISession, StatisticsAggregator> onBotStarted)
        {
            ...
            if (commandLine["latlng"] != null && commandLine["latlng"].Length > 0)
            {
                ...
                try
                {
                    ...
                    _session.Settings.AccountLatitude = lat;
                    _session.Settings.AccountLongitude = lng;
            ...
            if (settings.GPXConfig.UseGpxPathing)
            {
                                        ...
                                        (pt, ptindex) =>
                                            new
                                            {
                                                ...
                                                Distance = LocationUtils.CalculateDistanceInMeters(
                                                    _session.Settings.AccountLatitude,
                                                    _session.Settings.AccountLongitude,
                ...
                if (nearestPt != null)
                {
                    _session.Settings.AccountLatitude = nearestPt.Latitude;
                    _session.Settings.AccountLongitude = nearestPt.Longitude;
                ...
                if (apiCfg.UsePogoDevAPI || apiCfg.UseCustomAPI)
                {
                    ...
                    try
                    {
                        ...
                        if (settings.Auth.APIConfig.UseCustomAPI)
                            ...
                            urlcheck = $"https://pokehash.buddyauth.com/{Configuration.HashEndpoint}";
            ...
            stats.DirtyEvent +=
                () =>
                {
                    bool x = _session.Client.ClientSession.Player.Warn;
                    string warn = x ? "*(Flagged)*-" : null;

                    SetStatusText($"[RocketBot2 v{strVersion}] Team: {_session.Client.ClientSession.Player.Data.Team} -  {warn}" +
                ...
                foreach (var _bot in accountManager.AccountsReadOnly.OrderByDescending(p => p.Level).ThenByDescending(p => p.CurrentXp))
                {
                    ...
                    _item.Click += delegate
                    {
                        if (!Instance._botStarted)
                        {
                            ...
                            _session.Settings.AccountLatitude = _bot.AccountLatitude;
                            _session.Settings.AccountLongitude = _bot.AccountLongitude;
