...
namespace PoGo.NecroBot.Logic.State
{
    public class LoadSaveState : IState
    {
        public async Task<IState> Execute(ISession session, CancellationToken cancellationToken)
        {
            ...
            {
                Latitude = session.Client.ClientSession.Player.Latitude,
                Longitude = session.Client.ClientSession.Player.Longitude
            });

            session.EventDispatcher.Send(new WarnEvent
            {
                Message =
                    session.Translation.GetTranslation(TranslationString.WelcomeWarning, session.Client.ClientSession.Player.Latitude,
        ...
        public static void SaveLocationToDisk(ISession session, double lat, double lng, double speed)
        {
            ...
            if (LocationUtils.CalculateDistanceInMeters(lat, lng, session.Settings.AccountLatitude, session.Settings.AccountLongitude) > session.LogicSettings.MaxTravelDistanceInMeters)
                return;
