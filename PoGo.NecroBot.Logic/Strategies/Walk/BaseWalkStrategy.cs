...
namespace PoGo.NecroBot.Logic.Strategies.Walk
{
    abstract class BaseWalkStrategy : IWalkStrategy
    {
        ...
        public void OnStartWalking(ISession session, IGeoLocation desination, double calculatedDistance = 0.0)
        {
            ...
            if (distance == 0)
            {
                distance = CalculateDistance(session.Client.ClientSession.Player.Latitude, session.Client.ClientSession.Player.Longitude,
        ...
        public async Task DoWalk(List<GeoCoordinate> points, ISession session,
            ...
            CancellationToken cancellationToken, double walkSpeed = 0.0)
        {
            var alt = new GeoCoordinate(_client.ClientSession.Player.Latitude, _client.ClientSession.Player.Longitude).Altitude;
            ...
            {
                currentLocation = new GeoCoordinate(_client.ClientSession.Player.Latitude, _client.ClientSession.Player.Longitude);
