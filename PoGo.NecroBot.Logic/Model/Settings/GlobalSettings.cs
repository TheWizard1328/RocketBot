    ...
    public class GlobalSettings
    {
       ...
       [NecroBotConfig(SheetName = "HumanlikeDelays", Description = "Define the delays for humanlike behaviour when catching pokemon")]
            ...
            IElevationService elevationService, string configPath)
        {
            ...
            SetupConfig(newSession.Translation, session);
        ...
        private static void SetupConfig(ITranslation translator, ISession session)
        {
            if (false == PromptForBoolean(
                    translator,
                    translator.GetTranslation(
            ...
            while (true)
            {
                try
                {
                    ...
                    if (strSplit.Length > 1)
                    {
                        ...
                        session.Settings.AccountLatitude = dblLat;
                        session.Settings.AccountLongitude = dblLong;
