    ...
    public class UseItemMoveRerollTask
    {
        public static async Task Execute(ISession session, ItemData item, PokemonData pokemondata)
        {
            var response = await session.Client.ClientSession.RpcClient.SendRemoteProcedureCallAsync(new Request
            {
                ...
                RequestMessage = new UseItemMoveRerollMessage
                {
                    ItemId = item.ItemId,
            ...
            UseItemMoveRerollResponse useItemMoveRerollResponse = null;

            useItemMoveRerollResponse = UseItemMoveRerollResponse.Parser.ParseFrom(response);
            
            switch (useItemMoveRerollResponse.Result)
            {
