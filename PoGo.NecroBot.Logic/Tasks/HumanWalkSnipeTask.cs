...
namespace PoGo.NecroBot.Logic.Tasks
{
    ...
    public partial class HumanWalkSnipeTask
    {
        ...
        public static async Task ExecuteFetchData(ISession session)
        {
            ...
            await FetchData(_session.Client.ClientSession.Player.Latitude, _session.Client.ClientSession.Player.Longitude, true).ConfigureAwait(false);
        }
            ...
            FortData originalPokestop, FortDetailsResponse fortInfo)
        {
            ...
            do
            {
                ...
                pokemon = await GetNextSnipeablePokemon(session.Client.ClientSession.Player.Latitude, session.Client.ClientSession.Player.Longitude,
                    !caughtAnyPokemonInThisWalk).ConfigureAwait(false);
                if (pokemon != null)
                {
                    ...
                    await session.Navigation.Move(
                        ...
                        {
                            cancellationToken.ThrowIfCancellationRequested();
                            await ActionsWhenTravelToSnipeTarget(session, cancellationToken, pokemon, catchPokemon, spinPokestop).ConfigureAwait(false);
                        },
                        session,
                        cancellationToken,
                    ...
                    //await Task.Delay(pokemon.Setting.DelayTimeAtDestination, cancellationToken).ConfigureAwait(false);
                    await CatchNearbyPokemonsTask
                        .Execute(session, cancellationToken, pokemon.PokemonId, pokemon.Setting.AllowTransferWhileWalking).ConfigureAwait(false);
            ...
            if (caughtAnyPokemonInThisWalk && (!_setting.HumanWalkingSnipeAlwaysWalkBack || _setting.UseGpxPathing))
            {
                ...
                {
                    await WalkingBackGPXPath(session, cancellationToken, originalPokestop, fortInfo).ConfigureAwait(false);
                }
                else
                    await UpdateFarmingPokestop(session, cancellationToken).ConfigureAwait(false);
            }
        }
            ...
            FortData originalPokestop, FortDetailsResponse fortInfo)
        {
            ...
            await session.Navigation.Move(destination,
                async () =>
                {
                    ...
                    await UseNearbyPokestopsTask.SpinPokestopNearBy(session, cancellationToken).ConfigureAwait(false);
                },
                session,
                cancellationToken).ConfigureAwait(false);
        }
        ...
        private static async Task UpdateFarmingPokestop(ISession session, CancellationToken cancellationToken)
        {
            ...
            if (nearestStop != null)
            {
                ...
                if (walkedDistance > session.LogicSettings.HumanWalkingSnipeWalkbackDistanceLimit)
                {
                    ...
                    var nearbyPokeStops = await UseNearbyPokestopsTask.UpdateFortsData(session).ConfigureAwait(false);
        ...
        static void CalculateDistanceAndEstTime(SnipePokemonInfo p)
        {
            ...
            p.Distance = CalculateDistanceInMeters(
                _session.Client.ClientSession.Player.Latitude,
                _session.Client.ClientSession.Player.Longitude,
                p.Latitude,
                p.Longitude
            );
        ...
        private static async Task<SnipePokemonInfo> GetNextSnipeablePokemon(double lat, double lng, bool refreshData = true)
        {
            if (refreshData)
            {
                await FetchData(lat, lng).ConfigureAwait(false);
            }
        ...
        private static async Task FetchData(double lat, double lng, bool silent = false)
        {
            ...
            if (_setting.HumanWalkingSnipeIncludeDefaultLocation
                && LocationUtils.CalculateDistanceInMeters(lat, lng, _session.Settings.AccountLatitude, _session.Settings.AccountLongitude) > 1000)
            {
                allTasks.Add(FetchFromPokeradar(_session.Settings.AccountLatitude, _session.Settings.AccountLongitude));
                allTasks.Add(FetchFromSkiplagged(_session.Settings.AccountLatitude, _session.Settings.AccountLongitude));
                allTasks.Add(FetchFromPokecrew(_session.Settings.AccountLatitude, _session.Settings.AccountLongitude));
                allTasks.Add(FetchFromPokesnipers(_session.Settings.AccountLatitude, _session.Settings.AccountLongitude));
                allTasks.Add(FetchFromPokeZZ(_session.Settings.AccountLatitude, _session.Settings.AccountLongitude));
                allTasks.Add(FetchFromFastPokemap(_session.Settings.AccountLatitude, _session.Settings.AccountLongitude));
                allTasks.Add(FetchFromPokeWatcher(_session.Settings.AccountLatitude, _session.Settings.AccountLongitude));
            }
            ...
            await PostProcessDataFetched(fetchedPokemons, !silent).ConfigureAwait(false);
        }
