...
namespace PoGo.NecroBot.Logic.Tasks
{
    public class EvolvePokemonTask
    {
        ...
        public static async Task UseLuckyEgg(ISession session)
        {
            ...
            UseItemXpBoostResponse responseLuckyEgg = null;

            responseLuckyEgg = UseItemXpBoostResponse.Parser.ParseFrom(response);
            ...
            foreach (var pokemon in pokemonToEvolve)
            {
                ...
                if (await session.Inventory.CanEvolvePokemon(pokemon, filter).ConfigureAwait(false))
                {
                    try
                    {
                        ...
                        var item = filter == null ? ItemId.ItemUnknown : await GetRequireEvolveItem(session, pokemon.PokemonId, filter.EvolveToPokemonId);
                        var response = await session.Client.ClientSession.RpcClient.SendRemoteProcedureCallAsync(new Request
                        {
                            ...
                            RequestMessage = new EvolvePokemonMessage
                            {
                                ...
                                EvolutionItemRequirement = item
                            }.ToByteString()
                        }, true);

                        EvolvePokemonResponse evolveResponse = null;

                        evolveResponse = EvolvePokemonResponse.Parser.ParseFrom(response);
