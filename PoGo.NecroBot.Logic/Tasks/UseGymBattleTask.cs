...
namespace PoGo.NecroBot.Logic.Tasks
{
    public class UseGymBattleTask
    {
        ...
        public static async Task Execute(ISession session, CancellationToken cancellationToken, FortData gym, FortDetailsResponse gymInfo, GymGetInfoResponse gymDetails)
        {
            ...
            var distance = Session.Navigation.WalkStrategy.CalculateDistance(Session.Client.ClientSession.Player.Latitude, Session.Client.ClientSession.Player.Longitude, Gym.Latitude, Gym.Longitude);
        ...
        private async static Task StartRaidAttackLogic()
        {
            var response = await Session.Client.ClientSession.RpcClient.SendRemoteProcedureCallAsync(new Request
            {
                ...
                RequestMessage = new GetRaidDetailsMessage
                {
                    ...
                    RaidSeed = Gym.RaidInfo.RaidSeed
                }.ToByteString()
            }, true);

            GetRaidDetailsResponse RaidDetails = null;

            RaidDetails = GetRaidDetailsResponse.Parser.ParseFrom(response);
        ...
        private static async Task StartGymAttackLogic()
        {
            ...
            if (_defenders.Count() < 1 || Gym.OwnedByTeam == Session.Client.ClientSession.Player.Data.Team)
            {
                return;
            }
        ...
        private static async Task RevivePokemon(PokemonData pokemon)
        {
            ...
            if ((healPower >= pokemon.StaminaMax / 2 || maxRevives == 0) && normalRevives > 0 && pokemon.Stamina <= 0)
            {
                ...
                UseItemReviveResponse ret = null;

                ret = UseItemReviveResponse.Parser.ParseFrom(response);
        ...
        private static async Task<bool> UsePotion(PokemonData pokemon, int normalPotions)
        {
            ...
            UseItemPotionResponse ret = null;

            ret = UseItemPotionResponse.Parser.ParseFrom(response);
        ...
        private static async Task<List<BattleAction>> AttackGym(GymStartSessionResponse startResponse)
        {
            ...
            while (true)
            {
                ...
                var response = await Session.Client.ClientSession.RpcClient.SendRemoteProcedureCallAsync(new Request
                {
                    ...
                    {
                        GymId = Gym.Id,
                        AttackActions = { attackActionz },
                        BattleId = startResponse.Battle.BattleId,
                        LastRetrievedAction = a2,
                        PlayerLatDegrees = Session.Client.ClientSession.Player.Latitude,
                        PlayerLngDegrees = Session.Client.ClientSession.Player.Longitude,
                ...
                attackResult = GymBattleAttackResponse.Parser.ParseFrom(response);
                ...
                try
                {
                    var response = await Session.Client.ClientSession.RpcClient.SendRemoteProcedureCallAsync(new Request
                    {
                        ...
                        RequestMessage = new GymStartSessionMessage
                        {
                            ...
                            DefendingPokemonId = defenderId,
                            PlayerLatDegrees = Session.Client.ClientSession.Player.Latitude,
                            PlayerLngDegrees = Session.Client.ClientSession.Player.Longitude
                        }.ToByteString()
                    }, true);

                    result = GymStartSessionResponse.Parser.ParseFrom(response);
                }
                catch (Exception ex)
                {
        ...
        private static async Task EnsureJoinTeam(PlayerData player)
        {
            if (Session.Profile.PlayerData.Team == TeamColor.Neutral)
            {
                ...
                SetPlayerTeamResponse teamResponse = null;

                teamResponse = SetPlayerTeamResponse.Parser.ParseFrom(response);
