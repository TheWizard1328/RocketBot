    ...
    public class UseNearbyPokestopsTask
    {
        ...
        public static async Task Execute(ISession session, CancellationToken cancellationToken)
        {
            ...
            while (pokeStop != null)
            {
                ...
                var fortInfo = new FortDetailsResponse();

                if (pokeStop.Id.StartsWith(SetMoveToTargetTask.TARGET_ID))
                ...
                else
                {
                    ...
                    fortInfo = FortDetailsResponse.Parser.ParseFrom(response);
                }
                ...
                if (pokeStop.Type == FortType.Gym)
                {
                    var response = await session.Client.ClientSession.RpcClient.SendRemoteProcedureCallAsync(new Request
                    {
                        ...
                        RequestMessage = new GymGetInfoMessage
                        {
                            ...
                            PlayerLatDegrees = session.Client.ClientSession.Player.Latitude,
                            PlayerLngDegrees = session.Client.ClientSession.Player.Longitude                           
                        }.ToByteString()
                    }, true);

                    GymGetInfoResponse fortDetails = null;

                    fortDetails = GymGetInfoResponse.Parser.ParseFrom(response);
        ...
        private static async Task WalkingToPokeStop(ISession session, CancellationToken cancellationToken, FortData pokeStop, FortDetailsResponse fortInfo)
        {
            var distance = LocationUtils.CalculateDistanceInMeters(session.Client.ClientSession.Player.Latitude,
                    session.Client.ClientSession.Player.Longitude, pokeStop.Latitude, pokeStop.Longitude);
        ...
        public static async Task<FortData> GetNextPokeStop(ISession session)
        {
            ...
            if (session.LogicSettings.UseGpxPathing)
            {
                forts = forts.Where(p => LocationUtils.CalculateDistanceInMeters(p.Latitude, p.Longitude, session.Client.ClientSession.Player.Latitude, session.Client.ClientSession.Player.Longitude) < 40).ToList();
            }
            ...
            if (session.LogicSettings.GymConfig.PrioritizeGymOverPokestop)
            {
                ...
                var gyms = forts.Where(x => x.Type == FortType.Gym &&
                    LocationUtils.CalculateDistanceInMeters(x.Latitude, x.Longitude, session.Client.ClientSession.Player.Latitude, session.Client.ClientSession.Player.Longitude) < session.LogicSettings.GymConfig.MaxDistance);
                //.OrderBy(x => LocationUtils.CalculateDistanceInMeters(x.Latitude, x.Longitude, session.Client.ClientSession.Player.Latitude, session.Client.ClientSession.Player.Longitude));
                ...
                if (session.LogicSettings.GymConfig.PrioritizeGymWithFreeSlot)
                {
                    ...
                    foreach (var gym in gyms)
                    {
                        if (gym.OwnedByTeam == session.Profile.PlayerData.Team)
                        {
                            ...
                            var task = GymGetInfoResponse.Parser.ParseFrom(response);
            ...
            if (session.Forts.Count() > 0)
            {
                ...
                if (spinablePokestops.Count() > 0)
                {
                    foreach (var pokeStop in spinablePokestops)
                    {
                        ...
                        var fortInfo = FortDetailsResponse.Parser.ParseFrom(response);
        ...
        public static async Task FarmPokestop(ISession session, FortData pokeStop, FortDetailsResponse fortInfo, CancellationToken cancellationToken, bool doNotRetry = false)
        {
            ...
            var distance = LocationUtils.CalculateDistanceInMeters(pokeStop.Latitude, pokeStop.Longitude, session.Client.ClientSession.Player.Latitude, session.Client.ClientSession.Player.Longitude);
            ...
            if (distance < 50) //if (distance > 30)
            {
                ...
                await session.Client.RandomAPICall().ConfigureAwait(false);
            }
            ...
            do
            {
                ...
                do
                {
                    ...
                    fortSearch = FortSearchResponse.Parser.ParseFrom(response);
                ...
                else
                {
                    ...
                    session.EventDispatcher.Send(new FortUsedEvent
                    {
                        ...
                        Latitude = pokeStop.Latitude,
                        Longitude = pokeStop.Longitude,
                        Altitude = await session.ElevationService.GetElevation(session.Client.ClientSession.Player.Latitude, session.Client.ClientSession.Player.Longitude).ConfigureAwait(false),
                    ...
                    if (fortSearch.Result == FortSearchResponse.Types.Result.Success)
                    {
                        ...
                        // Update the cache
                        var fortFromCache = session.Client.ClientSession.Map.Cells.SelectMany(x => x.Forts).FirstOrDefault(f => f.Id == pokeStop.Id);
        ...
        public static async Task<List<FortData>> UpdateFortsData(ISession session)
        {
            ...
            if (!session.Client.LoggedIn)
                return new List<FortData>();

            var mapObjects = session.Client.ClientSession.Map.Cells;

            session.AddForts(mapObjects.SelectMany(p => p.Forts).ToList());
            ...
            var pokeStops = mapObjects.SelectMany(i => i.Forts)
                .Where(
                    i =>
                        ...
                        (
                            LocationUtils.CalculateDistanceInMeters(
                                session.Settings.AccountLatitude, session.Settings.AccountLatitude,
