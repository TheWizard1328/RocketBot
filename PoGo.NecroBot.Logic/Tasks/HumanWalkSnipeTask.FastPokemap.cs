    ...
    public partial class HumanWalkSnipeTask
    {
        ...
        public static async Task StartFastPokemapAsync(ISession session, CancellationToken cancellationToken)
        {
            ...
            while (true)
            {
                ...
                double lat = session.Client.ClientSession.Player.Latitude;

                double lng = session.Client.ClientSession.Player.Longitude;
                await OffsetScanFPM(scanOffset, step, lat, lng).ConfigureAwait(false);
                if(LocationUtils.CalculateDistanceInMeters(session.Client.ClientSession.Player.Latitude, session.Client.ClientSession.Player.Longitude, defaultLatitude, defaultLongitude) >1000)
                {
            ...
            taskDataLive = Task.Run(() =>
            {
                while (true)
                {
                    try
                    {
                        ...
                        do
                        {
                            var lat = _session.Client.ClientSession.Player.Latitude;
                            var lng = _session.Client.ClientSession.Player.Longitude;
