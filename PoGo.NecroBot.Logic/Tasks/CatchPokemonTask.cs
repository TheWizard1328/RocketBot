                ...
                do
                {
                    ...
                    if (CatchFleeContinuouslyCount >= 3 && session.LogicSettings.ByPassCatchFlee)
                    {
                        ...
                        for (int i = 0; i < ballToByPass.Count - 1; i++)
                        {
                            ...
                            ulong pokeId = encounter is EncounterResponse || encounter is IncenseEncounterResponse ? pokemon.EncounterId : _encounterId;
                            string spawid = encounter is EncounterResponse || encounter is IncenseEncounterResponse ? pokemon.SpawnPointId : currentFortData.Id;
                            ...
                            var response = await session.Client.ClientSession.RpcClient.SendRemoteProcedureCallAsync(new Request
                            {
                                ...
                                RequestMessage = new CatchPokemonMessage
                                {
                                    ...
                                    EncounterId = pokeId,
                                    SpawnPointId = spawid,
                                    Pokeball = ballToByPass[i],
                                    NormalizedReticleSize = 1.0,
                                    NormalizedHitPosition = 1.0,
                                    HitPokemon = !catchMissed,
                                    SpinModifier = 1
                                }.ToByteString()
                            }, true);
                            ...
                            Logger.Write("Use ARBonus values", LogLevel.Info);
                    ...
                    {
                        var pokeId = encounter is EncounterResponse || encounter is IncenseEncounterResponse ? pokemon.EncounterId : _encounterId;
                        var spawId = encounter is EncounterResponse || encounter is IncenseEncounterResponse ? pokemon.SpawnPointId : currentFortData.Id;

                        var arValues = new ARPlusEncounterValues
                        {
                            Awareness = (float)0.000,
                            PokemonFrightened = false,
                            Proximity = (float)0.000
                        ...
                        var response = await session.Client.ClientSession.RpcClient.SendRemoteProcedureCallAsync(new Request
                        {
                            ...
                            RequestMessage = new CatchPokemonMessage
                            {
                                ArPlusValues = arValues,
                                EncounterId = pokeId,
                                HitPokemon =hitPokemon,
                                NormalizedHitPosition = 1.95,
            ...
            CancellationToken cancellationToken)
        {
            ...
            foreach (var item in itemToUses)
            {
                          ...
                          pokemonLv >= filter.UseItemMinLevel)))
                {
                    ...
                    //berry.Count -= 1;
                    if (useItemEncounterResponse.Status == UseItemEncounterResponse.Types.Status.Success)
                    {
                    ...
                    {
                        Logger.Debug($"Use berries result : {useItemEncounterResponse.Status}");
                    }
                }
