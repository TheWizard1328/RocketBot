...
namespace PoGo.NecroBot.Logic.Captcha
{
    public class CaptchaManager
    {
        ...
        private static async Task<string> GetNewCaptchaURL(ISession session)
        {
            ...
            CheckChallengeResponse checkChallengeResponse = null;

            checkChallengeResponse = CheckChallengeResponse.Parser.ParseFrom(response);

            if (checkChallengeResponse.ShowChallenge)
            {
                return checkChallengeResponse.ChallengeUrl;
            }
            return string.Empty;
        }
